...
DLL一覧（Unity移植時に把握すべきものを全列挙）

> 「このリポジトリ内で直接確認できるDLL」と「csproj上で要求されるDLL（NuGet含む）」を分けて記載。

### A. リポジトリ内に実体があるDLL
- `AssetStudio.GUI/Libraries/OpenTK.WinForms.dll`

### B. ネイティブDLL（csprojでコピー/配置が明示）
- `AssetStudio.FBXNative.dll`（x86/x64）
- `fmod.dll`（x86/x64）
- `acl.dll`（x86/x64）
- `sracl.dll`（x86/x64）
- `acldb.dll`（x86/x64）
- `HLSLDecompiler.dll`（x86/x64）
- （補足）`BinaryDecompiler.lib` はDLLではなく `.lib` です

### C. コード上のP/Invokeで要求されるDLL名
- `AssetStudio.FBXNative`（`FbxDll.DllName`）
- `libfbxsdk`（`FbxDll.FbxsdkDllName`）
- `acl`
- `sracl`
- `acldb`
- `fmod` / `fmod64`（`FMOD.VERSION.dll`）
- `kernel32.dll`（WindowsのDLLロードAPI）
- `libdl`（Linux/macOSのDLLロードAPI）

### D. NuGet由来のマネージドDLL（ビルド時に解決）
#### AssetStudio
- `MessagePack`
- `Newtonsoft.Json`
- `ZstdSharp.Port`
...
#### AssetStudio.Utility
- `Kyaru.Texture2DDecoder`
- `Kyaru.Texture2DDecoder.Windows`
- `Mono.Cecil`
...
- `Newtonsoft.Json`
- `OpenTK`
- `OpenTK.WinForms`（HintPath参照）

### E. Unityへ取り込む時の実務チェック（DLLのみ）
1. `acl/sracl/acldb/fmod/AssetStudio.FBXNative/HLSLDecompiler` を `Assets/Plugins/x86` と `Assets/Plugins/x86_64` に配置。
2. `OpenTK.WinForms.dll` はWinForms依存のため、Unity Editor拡張でWinFormsを使わないなら基本不要。
3. `libfbxsdk` は `AssetStudio.FBXNative` の依存として同梱が必要な場合あり（実行時エラーで不足DLL名を確認）。
4. Linux/macOSで動かす場合は `libacl.so` / `libsracl.so` / `libacldb.so` / `libfmod.so` など `DllLoader` 規約（`lib<name>.so/.dylib`）に合わせる。


---

バンドル読み取り処理とアニメ変換処理の**二分**（責務分離）
